			...
			if (!is_dir($dirpath)){
				try {
					if (@mkdir($dirpath)){
						...
						$permissions['dir'][$dirname] = 'not exists';
...
//			$this->getRequest()->isPost() && $this->_forward('step2');
	...
	private function _setupDatabase($dbinfo){
		...
		try {
			...
			$file = file_get_contents(APPLICATION_PATH.'installer/resourses/seotoaster.sql');
			if ($file === false){
				return false;
			}
			$queries = Tools_System_SqlSplitter::split($file);
	...
	private function _findLanguages() {
        ...
        $flags = glob('system/images/flags/*.png');

        foreach ($flags as $flag) {
            $country = substr(Tools_Filesystem_Tools::basename($flag), 0, 2);
			$locale = new Zend_Locale(Zend_Locale::getLocaleToTerritory($country));
			$lang = $locale->getLanguage();
            if (array_key_exists($lang, $availLanguages)){
                $availLanguages[$lang] = $flag;
            }
